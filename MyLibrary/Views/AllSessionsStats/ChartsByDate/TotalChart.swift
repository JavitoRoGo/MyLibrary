...
struct TotalChart: View {
    ...
    var pages: [Int] {
        model.calcTotalPagesPerYear().pages
    }
    var years: [String] {
        model.calcTotalPagesPerYear().years
    }
    var max: Int {
        pages.max() ?? 0
    }
    ...
    var body: some View {
        Chart(animateDatas) { data in
            ...
            if let currentActiveItem, currentActiveItem.id == data.id {
                RuleMark(x: plotLocationX, yStart: .value("inicio", 0), yEnd: .value("Fin", max + 5000))
                    ...
                    .gesture(
                        DragGesture()
                            .onChanged { value in
                                ...
                                if let yearString: String = proxy.value(atX: location.x) {
                                    if let currentItem = animateDatas.first(where: { item in
                                        item.label == yearString }) {
        ...
        .task {
            animateDatas = getValuesIntoDataArrayForAnimateGraph(pages, years)
            animateGraph()
        }
    }
